...
function ToolPartView({
  ...
  if ("approval" in part && part.state === "approval-requested") {
    ...
    const isDelete = toolName === "deleteResource";
    const deleteId = (part.input as { id?: string } | undefined)?.id;
    return (
      <div className="flex justify-start">
        <Surface className="max-w-[85%] px-4 py-3 rounded-xl ring-2 ring-kumo-warning">
          <div className="flex items-center gap-2 mb-2">
            ...
            <Text size="sm" bold>
              {isDelete
                ? `This will delete ${deleteId}. Confirm?`
...
function Chat() {
  ...
  const agent = useAgent<InfraAgent>({
    agent: "InfraAgent",
    ...
    // Workflow progress + "resources changed" notices from the agent.
    onMessage: provisioning.onMessage
  });

  // A stable function for the panel's effect. useAgent returns a fresh object
  // on every render; depending on it directly would refetch in a loop.
  const agentRef = useRef(agent);
  agentRef.current = agent;
  const loadResources = useCallback(
    () => agentRef.current.stub.getResources(),
    []
  );
            ...
            <Empty
              ...
              contents={
                <div className="flex flex-wrap justify-center gap-2">
                  {[
                    "What's running?",
                    "Create a redis instance for dev",
                    "Delete redis-dev-01"
                  ].map((prompt) => (
                    <Button
                      key={prompt}
